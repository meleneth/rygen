...
{
  Projection = glm::ortho( 0.0f, (float)video.width, 0.0f, (float)video.height, -5.0f, 5.0f);
  View  = glm::mat4(1.0f);
...
void Demo::render_frame() 
{
  ...
  GLuint MatrixID  = glGetUniformLocation(texture_shader->shader_program_id, "MVP");
  GLuint TextureID = glGetUniformLocation(texture_shader->shader_program_id, "mytexture");

  // vertex pointer
  glVertexAttribPointer((GLuint)0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (const GLvoid*)0);
  glEnableVertexAttribArray(0);

  // UV pointer
  glVertexAttribPointer((GLuint)1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (const GLvoid*)8);
  ...
  glm::mat4 location_matrix = glm::translate(glm::mat4(1.0f), glm::vec3(x, y, 0.f));
  glm::mat4 scale_matrix = glm::scale(glm::mat4(1.0f), glm::vec3(width, height, 0.0f));
