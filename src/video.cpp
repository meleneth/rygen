...
Video::Video(int width, int height) {
  ...
  console << "[WidgetRenderer] WidgetRenderer::WidgetRenderer() finished"
          << endl;
}
...
void Video::setup() {
  ...
  // Create a window. Window mode MUST include SDL_WINDOW_OPENGL for use with
  // OpenGL.
  window = SDL_CreateWindow(".sapien.", SDL_WINDOWPOS_CENTERED,
